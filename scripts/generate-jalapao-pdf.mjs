/**
 * Gera PDF consolidado dos roteiros do Jalapão a partir dos dados extraídos das imagens.
 * Uso: npm run generate:jalapao-pdf
 */
...
import { packageInfo, routes } from "./jalapao-routes-data.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.resolve(__dirname, "..");
const OUT_DIR = path.join(ROOT, "public", "downloads");
const OUT_FILE = path.join(OUT_DIR, "roteiro-jalapao-completo.pdf");
const COVER_IMAGE = path.join(
  ROOT,
  "Site",
  "Site_",
  "Fotos jalapão_",
  "9B9E581E-96FD-4140-BC38-213AF064C22B.jpg",
);
    ...
    <section class="route-section" id="${route.id}">
      ...
      <div class="route-meta">
        <div class="meta-item">
          ...
          <span><strong>Bases:</strong> ${esc(route.base)}</span>
        </div>
      </div>
...
<head>
  <meta charset="UTF-8" />
  <title>Roteiros do Jalapão — Gaviões do Cerrado</title>
  <style>
    @page {
      size: A4;
...
<body>
  <div class="cover">
    ...
    <div class="cover-content">
      <p class="cover-kicker">Ecoturismo &amp; aventura no Cerrado</p>
      <h1><span>Roteiros</span> do Jalapão</h1>
      <p class="cover-tagline">Guia completo com todas as opções de expedição — de 2 a 6 dias pelos ícones do Parque Estadual.</p>
      <div class="cover-stats">
        <div class="cover-stat"><strong>5</strong><span>Roteiros</span></div>
        <div class="cover-stat"><strong>2–6</strong><span>Dias</span></div>
        <div class="cover-stat"><strong>30+</strong><span>Atrativos</span></div>
      </div>
    </div>
  </div>
  ...
  <div class="package-page" id="pacote">
    ...
    <p class="package-note">
      <strong>Observação:</strong> Atividades marcadas como opcionais (rafting, tirolesa, rapel, trekking nascer do sol) podem ser contratadas à parte. Horários e ordem dos passeios podem ser ajustados conforme condições climáticas, disponibilidade e perfil do grupo.
    </p>
  </div>
  ...
  <div class="footer-page">
    <h2>Pronto para viver o Jalapão?</h2>
    <p>Fale conosco pelo WhatsApp e monte sua expedição ideal.</p>
    <p class="footer-url">www.jalapaogavioesdocerrado.com.br</p>
    <p class="footer-note">Jalapão Gaviões do Cerrado · Guia credenciado MTur · Tocantins, Brasil</p>
  </div>
</body>
</html>`;
}
...
async function main() {
  ...
  const tmpHtml = path.join(OUT_DIR, "_jalapao-preview.html");
  ...
  await page.pdf({
    ...
    footerTemplate: `
      <div style="width:100%;font-size:8px;color:#152238;padding:0 16mm;display:flex;justify-content:space-between;font-family:Segoe UI,sans-serif;">
        <span>Jalapão Gaviões do Cerrado</span>
