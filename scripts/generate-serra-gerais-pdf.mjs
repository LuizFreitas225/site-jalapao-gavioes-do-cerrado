/**
 * Gera PDF consolidado dos roteiros da Serra Gerais.
 * Uso: npm run generate:serra-gerais-pdf
 */
...
import { packageInfo, routes } from "./serra-gerais-routes-data.mjs";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.resolve(__dirname, "..");
const OUT_DIR = path.join(ROOT, "public", "downloads");
const OUT_FILE = path.join(OUT_DIR, "roteiro-serra-gerais-completo.pdf");
const COVER_IMAGE = path.join(ROOT, "public", "images", "routes", "serra-gerais-cover.jpg");
...
<head>
  <meta charset="UTF-8" />
  <title>Roteiros da Serra Gerais — Gaviões do Cerrado</title>
  <style>
    @page {
      size: A4;
...
<body>
  <div class="cover">
    ...
    <div class="cover-content">
      <p class="cover-kicker">Ecoturismo &amp; aventura no Tocantins</p>
      <h1><span>Roteiros</span> da Serra Gerais</h1>
      <p class="cover-tagline">Guia completo com opções de expedição — cânions, cachoeiras, rios cristalinos e mirantes do sul tocantinense.</p>
      <div class="cover-stats">
        ...
        <div class="cover-stat"><strong>12+</strong><span>Atrativos</span></div>
      </div>
    </div>
  </div>
  ...
  <div class="package-page" id="pacote">
    ...
    <p class="package-note">
      <strong>Observação:</strong> Horários e ordem dos passeios podem ser ajustados conforme condições climáticas, disponibilidade e perfil do grupo. O Cânion Encantado envolve trilha de aproximadamente 11 km com escadarias.
    </p>
  </div>
  ...
  <div class="footer-page">
    <h2>Pronto para viver a Serra Gerais?</h2>
...
async function main() {
  ...
  const tmpHtml = path.join(OUT_DIR, "_serra-gerais-preview.html");
  ...
  await page.pdf({
    ...
    footerTemplate: `
      <div style="width:100%;font-size:8px;color:#152238;padding:0 16mm;display:flex;justify-content:space-between;font-family:Segoe UI,sans-serif;">
        <span>Jalapão Gaviões do Cerrado · Serra Gerais</span>
